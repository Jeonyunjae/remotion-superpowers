...
## 에이전트 아키텍처 (서브에이전트 병렬 처리)

단일 에이전트가 모든 작업을 순차 처리하지 않습니다. **오케스트레이터 + 서브에이전트** 구조로 독립적인 작업을 병렬 처리합니다.
...
메인 에이전트 (오케스트레이터)
  ...
  ├─ 직접 수행 ──────────────────────────────────────────────
  │   문서 확인, 나레이션 생성, 음악 생성,
  │   Remotion 코드 작성, 프리뷰, 렌더링
  │
  ├─ media-scout (서브에이전트) ─── 비주얼 에셋 병렬 수집 ───
  │   ├→ 스톡 영상 검색 (Pexels)
  │   ├→ AI 이미지 생성 (config.yaml 프로바이더)
  │   └→ AI 영상 클립 생성 (config.yaml 프로바이더)
  │       ↑ 3개 작업 동시 수행
  │
  └─ post-producer (서브에이전트) ── 포스트프로덕션 병렬 검증 ──
      ├→ AI 영상 리뷰 (비주얼/페이싱/싱크/플랫폼)
      ├→ QC 기술 검증 (코덱/해상도/LUFS/파일크기)
      └→ 접근성 검사 (자막/색상대비/깜빡임)
          ↑ 3개 검증 동시 수행
```
...
`/create-video` 및 `/create-short` 실행 시:

| Phase | 작업 | 수행 주체 | 방식 |
|-------|------|----------|------|
| 1 | 문서 확인 + 설정 | 메인 | 순차 |
| 2a | 나레이션 생성 | 메인 | 순차 (타이밍 기준) |
| 2b-c | 배경음악 + 효과음 | 메인 | **병렬** |
| 3 | 스톡 + 이미지 + 영상 수집 | **media-scout** | **병렬** |
| 4 | Remotion 코드 작성 | 메인 | 순차 |
| 5 | 프리뷰 + 피드백 | 메인 | 순차 |
| 6 | 렌더링 | 메인 | 순차 |
| 7 | AI 리뷰 + QC + 접근성 | **post-producer** | **병렬** |
| 8 | 수정 루프 | 메인 | 순차 |

> Phase 3과 Phase 7에서 서브에이전트가 각각 3개 작업을 동시에 처리하여 전체 제작 시간을 단축합니다.
